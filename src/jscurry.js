...
export default (function () {
  ...
  F.constant = function (x) {
    return function (/* ... */) {
      return x;
    };
  };
  // "const" is a reserved word in ECMAScript 3
  F.applyF = function (f, args) {
    return f.apply(undefined, args);
  }
  F.curry = function (f /* , ... */) {
    var g = f;
    arguments[0] = undefined;
    return g.bind.apply(g, arguments);
  };
  ...
  {
    return function (/* ... */) {
      return F.applyF(f, F.map(g, arguments));
    };
  };
  F.flip = function (f) {
    return function (x, y) {
  ...
  F.cor = function (/* ... */) {	// conditional or
    var fs = arguments;
    return function (/* ... */) {
      return F.any(F([F._, arguments], F.applyF), fs);
    };
  };
  F.aritize = function (n, f)	// for discarding optional trailing arguments
  {
    return function (/* ... */) {
      return F.applyF(f, sliceMF.call(arguments, 0, n));
    };
  };
  ...
  F.defOr = function (xP, y) {
    return xP !== undefined ? xP : y;
  };
  ...
  F.cmpX = function (x, y) {
    return x - y;
  };	// for finite numbers, or Dates
  F.cmpJS = function (s, t) {
  ...
  {
    return F.any(function (e, i) {
        return i == w.length ? 1 : cmpE(e, w[i]);
      }, v) ||
      v.length - w.length;
  };
  F.eqTo = function (x, cmpP) {
    if (!cmpP) cmpP = function (y, z) {
      return y !== z;
    };
  ...
  F.memoF = function (f, memo) {
    if (!memo) memo = {};
    return function (p) {
      return memo.hasOwnProperty(p) ? memo[p] : (memo[p] = f(p));
    };
  };
  F.replicate = function (n, e) {
    return F.fToA(F.constant(e), n);
  };
  F.setF = function (obj, p, v) {
    obj[p] = v;
  };
  F.obj1 = function (p, v) {
    var res = {};
  ...
  F.array = function (/* ... */) {
    return sliceMF.call(arguments, 0);
  };
  F.concatArgs = F.oMap(F('concat', []),
    function (a) {
      return Array.isArray(a) ? a : F.slice(a);
    });
  F.concatMap = function (f, a) {
    return F.applyF(F.concatArgs, F.map(f, a));
  };
  F.reverseCopy = function (a) {
    return F.slice(a).reverse();
  };
  ...
  F.elem = function (e, a, cmpP) {
    return F.elemIndex(e, a, cmpP) != -1;
  };
  F.all = function (qF, a) {
  ...
  F.unzip =	// matrix transpose, similar to Haskell unzip/unzip3/unzip4/...
    function (zs) {
      return zs.length ? F.applyF(F.zip, zs) : [];
    };
  F.filter = function (qF, a) {
    if (a.filter && Array.isArray(a)) return a.filter(qF);
    return F.fold(function (y, e, i, a) {
      if (qF(e, i, a)) y.push(e);
      return y;
    }, a, []);
  };
  ...
  F.test = function (t) {	// e.g. for dynamic type checking when appropriate
    if (t === 0 || t === '') t = typeof t;
    if (typeof t == 'string') return function (x) {
      return typeof x == t;
    };
    if (t === Array || t === Date || t === RegExp)	// assumes same frame
      return function (x) {
        return x != null && x.constructor == t;
      };
    ...
    if (Array.isArray(t)) {
      if (t.length == 1) {
        t = F.test(t[0]);
        return function (x) {
          return Array.isArray(x) && F.all(t, x);
        };
      } else {	// "or" of tests
        t = F.map(F.test, t);
        return function (x) {
          return F.any(function (qF) {
            return qF(x);
          }, t);
        };
      }
    ...
    if (typeof t == 'object') {
      ...
      return function (x) {
        return x != null && F.all(function (k, i) {
          return fs[i](x[k]);
        }, ks);
      };
    }
